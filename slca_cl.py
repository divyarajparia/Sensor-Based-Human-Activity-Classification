...
class SLCA(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform, dataset=None):
        ...
        self.class_counts = torch.ones(self.num_classes)
    ...
    def end_task(self, dataset):
        ...
        class_priors = self.class_counts / (self.class_counts.sum() - (self.offset_2 - self.offset_1 + 1))
        self.log_priors = torch.log(class_priors).to(self.device)
    ...
    def forward(self, x):
        ...
        if (not self.training) and (self.log_priors is not None):
            adjusted_logits = logits + self.logit_adjustment_temperature * self.log_priors[:logits.size(1)]
